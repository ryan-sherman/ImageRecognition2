...
##    test_labels = extract_labels(test_labels_filename, 10000)
    ...
    with open("traindata.pkl") as f:
        shapemap, data_block = pickle.load(f)
        for shape in categories:
            raw_data += data_block[shape]

    TOTAL_SIZE = len(raw_data)

    #split pairs into separate arrays
    data, labels = zip(*raw_data)
    ...
    print("SIZE2 =========>",tmp.size)

    data = data[:,:,:,0].reshape(TOTAL_SIZE,32,32,1)
    ...
    train_labels = labels[:int(TOTAL_SIZE * .8)]
    test_labels = labels[int(TOTAL_SIZE * .8):]

    VALIDATION_SIZE = TOTAL_SIZE / 12
  ...
  def model(data, train=False):
    ...
    # Add a 10% dropout during training only. Dropout also scales
    # activations such that no rescaling is needed at evaluation time.
    if train:
      hidden = tf.nn.dropout(hidden, 0.1, seed=SEED)
